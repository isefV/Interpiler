...
void SYNTACTIC_M::parse(VEC<int>* token) {
	...
	for (int token_id = 0; token_id < token_size ; token_id++) {
		...
		int token_code = token->at(token_id);
	...
	for (int item = reomve_id.size() - 1; item >= 0; item--) {
		if(reomve_id.at(item) == token->size() - 1)
			token->pop_back();
		else
			token->erase(token->begin() + reomve_id.at(item));
	}
		...
		if ((code & MIDCODE_LEFT) == OP_SEPRATOR) {
			if (((code & MIDCODE_RIGHT) == OP_SEP_LEFT_PARENTHESE)
				&& (TOKEN_IS_KEYWORD(_keyword, KYW_IF)
					...
					|| TOKEN_IS_KEYWORD(_keyword, KYW_RET)
					|| TOKEN_IS_KEYWORD(_keyword, KYW_STOP)
					|| TOKEN_IS_KEYWORD(_keyword, KYW_NEXT)
...
VEC<EXPRESSION*>* SYNTACTIC_M::run(VEC<int>* tokens) {
	_keyword = KYW_NON;
	_paren_counter = 0;
	_brac_counter = 0;
	is_args = false;
